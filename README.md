...
系统采用前后端分离架构，注重界面的专业性（机构级暗黑交易面板）、底层数据的健壮性（防爬虫本地宽泛缓存）以及多维度的智能分析（大语言模型深度接入）。
...
- **全新数据库与多用户体系 (MySQL)**：
  ...
  - 用户配置、策略参数、自选股池、真实模拟交易流水（买卖点、仓位、成本）全部迁移至 MySQL 数据库，实现千人千面。
- **资产收益自动核算**：
...
- **机构级暗黑终端与独立视图**：
  - 全新侧边栏布局：分离为 `大盘总览`、`我的自选 (StockManager)` 与 `账户配置 (UserConfig)`，模块更加清晰。
...
- **高性能与防爬虫机制**：
  ...
  - **后端分页计算**：大盘总览采用按需分页计算，大幅降低 API 并发请求，提升海量自选股的加载速度。
...
- 登录后，进入左侧菜单的 **“账户配置 (UserConfig)”**。在这里您可以可视化地修改：
...
### 2. 我的自选与资产核算
- 进入 **“我的自选”** 页面，您可以添加任意 A 股、ETF 或场外基金。
- **交易记录录入**：点击列表中的“操作”按钮，录入真实买卖流水（价格、数量、日期），系统会自动同步您的“持仓成本”。
- **收益一键核算**：点击顶部的“计算收益”按钮，系统将根据实时行情与您的历史操作流水，自动为您计算当前的盈亏金额及收益率。

### 3. 大盘总览与步进推演
- **全局回测**：在“大盘总览”中设定全局回测区间。点击“执行回测”后，系统会自动按页拉取数据并展示股票池内标的在该区间的期末净值与收益率。
![alt text](image_3.png)
- **推演交互**：点击某只股票下钻进入暗黑推演终端。图表默认仅展示前 60 天数据作为预热期。点击右上角的 **[推演 30 天 ⏩]**，K 线图上将实时标注买卖点箭头，并在下方同步生成交易日志表格。
